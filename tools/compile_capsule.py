#!/usr/bin/env python3
"""
axm-embodied/tools/compile_capsule.py
...
Usage:
    python tools/compile_capsule.py <capsule_dir> <out_dir>
    python tools/compile_capsule.py <capsule_dir> <out_dir> --suite ed25519
    python tools/compile_capsule.py <capsule_dir> <out_dir> --gold

    # Compile all capsules in a directory:
    for d in capsules_final/*/; do
        python tools/compile_capsule.py "$d" "shards/$(basename $d)"
    done

Architecture:
    sim_robot_final.py  →  capsule/          (data generator, unchanged)
    compile_capsule.py  →  shard/            (this file — compilation)
