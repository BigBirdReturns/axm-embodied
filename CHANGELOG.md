...
Proof of *when*, and the page that drives the point home.

### Added
- **Incident attestation** (`src/axm_embodied/attest.py`) — a signature
  proves who sealed a shard, never when. Sealing a breach now queues an
  attestation entry: a byte-identical manifest copy (which commits to the
  Merkle root, which commits to every byte), its digests, and a
  pre-encoded RFC 3161 timestamp query. Queueing is offline (~2 KB); the
  TSQ DER encoding is dependency-free and byte-identical to
  `openssl ts -query -sha256 -cert -no_nonce` (asserted by test).
  `axm-runtime attest-flush` anchors pending entries at a TSA
  (default freetsa.org, mirroring the kernel's gold-shard attestation)
  and is best-effort by design — a robot in the field retries on the
  next flush. New CLI commands: `attest`, `attest-flush`.
- **Attestation shards** (`axm-runtime attest-publish`, genesis RFC 0005)
  — an anchored entry compiles into an ordinary signed v1 shard: the raw
  RFC 3161 query/response and target-manifest copy in `content/`, anchor
  metadata in `ext/attestations@1.jsonl` (with the TSA's `genTime`
  extracted from the DER response), and a `references@1` citation of the
  anchored shard. The proof-of-when travels the same rails as the record
  it covers, so the full evidence bundle for an incident is three linked
  shards: envelope ← incident ← attestation.
- Landing page: **"One Incident, Two Stories"** interactive demo — a
  NARRATIVE/DISK toggle over a real 100-frame fault flight generated by
  this repo's toolchain. Narrative mode shows the VLA's per-frame
  confidence (frame 50 indistinguishable from its neighbors); disk mode
  shows latent L∞ against the signed bound breaching 18× on that exact
  frame, with ESTOP annotation, hover inspection, a breach-window table,
  defense/plaintiff dual readings of the same shard, and the real chain
  of custody (envelope id → incident id → RFC 3161 anchor, verified OK).
